...
function aiRateLimiter(kind = 'allofy') {
  return async function limitAiRequest(req, res, next) {
    ...
    const now = Date.now();
    const minuteLimit = Number(process.env.AI_MINUTE_LIMIT || 8);
    const dailyLimit = Number(kind === 'allofy' ? process.env.ALLOFY_DAILY_LIMIT || 20 : process.env.AI_DAILY_LIMIT || 40);
    ...
    else if (bucket.count >= minuteLimit) {
      return res.status(429).json({ error: 'Muitas solicitações seguidas. Aguarde um minuto.', code: 'minute_limit' });
    } else bucket.count += 1;
